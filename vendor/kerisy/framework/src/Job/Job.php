...
class Job extends JobBase{
    ...
    public function saveNext($r){
        ...
        $affected = self::runUpdate(
            "INSERT INTO " . self::$dbPre.self::$tableName . " (handler, queue, run_at, created_at,schedule,tag) VALUES(?, ?, ?, '".$now."',?,?)",
            array(serialize($handler), (string) $this->queue, $run_at,$schedule,$tag)
        );
        if ($affected < 1) {
    ...
    public function getHandler() {
        $rs = $this->runQuery(
            "SELECT handler,schedule,tag FROM " . self::$dbPre.self::$tableName . " WHERE id = ?",
            array($this->job_id)
        );
        return current($rs);
    }
    ...
    public static function enqueue($handler, $queue = "default", $tag = null, $run_at = null, $schedule = null) {
        $config = config('database')->get('job');
        self::$dbPre = $config['prefix'];
...
//                self::log("[JOB] cron job has exist, failed to enqueue new job", self::ERROR);
        ...
        $affected = self::runUpdate(
            "INSERT INTO " . self::$dbPre . self::$tableName . " (handler, queue, run_at, created_at, schedule, tag) VALUES(?, ?, ?, '" . $now . "',?,?)",
            array(serialize($handler), (string)$queue, $run_at, $schedule, $tag)
        );
        if ($affected < 1) {
            self::log("[JOB] failed to enqueue new job", self::ERROR);
            return false;
